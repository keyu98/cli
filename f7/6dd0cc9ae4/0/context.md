...
I've noticed a pattern that I'd like to address:
...
Right now, we're assuming that a span finished when an error is encountered. While this is correct, this could mean that lower than expected latencies are being reported and a user looking at the logs...

### Prompt 30

Let's use C but with a little change: Let's use something like `RecordError` as the method name for that.
