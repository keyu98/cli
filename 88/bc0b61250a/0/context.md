...
I believe we now have "Git hooks can detect Copilot CLI subprocesses via the COPILOT_CLI=1 environment variable to skip interactive prompts" in the latest v0.0.421

this might be the solution to /debug-e2e the failing tests?

### Prompt 17

yes we need to add it to the detection, this is in the production code side yes?
...
add a github issue to do the interface change. it's specifically around the commit-hook prompt skip yes?

### Prompt 20

what would we set for claude's version of isAgentSubprocess()?

### Prompt 21

right so an optionally implemented interface?
...
now, back to /debug-e2e we've had two almost green runs...

### Prompt 25

let's gate these the same way as gemini, start with 4?

### Prompt 26

commit, let's see how this e2e run goes
