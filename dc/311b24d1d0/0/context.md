...
In displayRestoredSessions, is the additional check for     if sessions[i].CreatedAt.Equal(sessions[j].CreatedAt) { necessary or even correct? Wouldn't do a `sort.SliceStable()` alone work as well?
